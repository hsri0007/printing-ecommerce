...
export function UPIPayment({ amount, onPaymentSuccess, onPaymentError }: UPIPaymentProps) {
  ...
  const upiLink = `upi://pay?pa=merchant@upi&pn=Brahmani Graphics&am=${amount}&cu=INR&tn=Brahmani Graphics Order Payment`
  ...
  const handleUPIPayment = async (appId?: string) => {
    ...
    try {
      ...
      const paymentLink = appId
        ? `${appId}://upi/pay?pa=merchant@upi&pn=Brahmani Graphics&am=${amount}&cu=INR&tn=Order Payment&tr=${orderData.order.id}`
        : upiLink
